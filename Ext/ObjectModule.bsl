...
Процедура ПрименитьНастройку() Экспорт
	...
	Попытка
		...
		Если ЗначениеЗаполнено(Настройки) Тогда
			Если Настройки.Свойство("Сертификаты") Тогда
...
Процедура ВыполнитьОбработку() Экспорт
	...
	// Робота нельзя запускать в КЦ, кроме админа из формы
	// Нельзя запускать на филиале из формы не админу
	Если ЭтоКЦ И НЕ(ЗапускИзФормы И ЕстьПолныеПрава())
		ИЛИ НЕ ЭтоКЦ И ЗапускИзФормы И НЕ ЕстьПолныеПрава() Тогда
		ЗафиксироватьОшибку("Запуск робота запрещен!");
		Возврат;
	КонецЕсли;
	...
	ОтпечатокСертификата = ПолучитьСертификат(); // Получем сертификат по умолчанию (для синхронизации партий)
	...
	НачПериодаДокументы = НачалоДня(ТекущаяДата()) - 86400*ДнейСозданиеЗадач;
	...
	ОбновитьТаблицуДокументов(Параметры);
...
Процедура СинхронизироватьПартии(ВыбранныеЗадачи = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	(ВТАктуальныеПартии.НомерПартии ЕСТЬ NULL
		|			ИЛИ ВТПартииБД.НомерПартии ЕСТЬ NULL
		|			ИЛИ ВТАктуальныеПартии.Дата <> ВТПартииБД.Дата
		|			ИЛИ ВТАктуальныеПартии.ДатаИзготовления <> ВТПартииБД.ДатаИзготовления
		|			ИЛИ ВТАктуальныеПартии.МассаПервоначальная <> ВТПартииБД.МассаПервоначальная
		|			ИЛИ ВТАктуальныеПартии.МассаДоступная <> ВТПартииБД.МассаДоступная
		|			ИЛИ ВТАктуальныеПартии.Статус <> ВТПартииБД.Статус
		|			ИЛИ ВТАктуальныеПартии.ОКПД2 <> ВТПартииБД.ОКПД2
		|			ИЛИ ВТАктуальныеПартии.Производитель <> ВТПартииБД.Производитель
		|			ИЛИ ВТАктуальныеПартии.Фирма <> ВТПартииБД.Фирма
		|			ИЛИ ВТАктуальныеПартии.Manufacturer_Address <> ВТПартииБД.Manufacturer_Address
		|			ИЛИ ВТАктуальныеПартии.StoragePlace_Address <> ВТПартииБД.StoragePlace_Address)";
...
Процедура ОбновитьТаблицуПартий(Параметры, hasMore = Ложь) Экспорт
	...
	Для Каждого Record Из Records Цикл
		...
		НоваяСтрока.Дата = ПреобразоватьДатуИзXML(Record.DateRegistration);
		НоваяСтрока.ДатаИзготовления = ПреобразоватьДатуИзXML(Record.createDate);
		...
		ТекстОшибки = ЗаполнитьСтрокуПартии(НоваяСтрока); // ЖЕЛАТЕЛЬНО ВЫНЕСТИ ЗА ЦИКЛ
		...
		Если Record.ListValueQualityIndicator.Свойства().Получить("Value") <> Неопределено Тогда	
			...
			//ЧтениеJSON.УстановитьСтроку(НоваяСтрока.ДополнительныеСвойства);
...
Процедура ОбновитьТаблицуДокументов(Параметры) Экспорт
	
	ТаблицаДокументов.Очистить();
	
	СвойствоНеВыгружатьВЗерно = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Не выгружать в ФГИС Зерно",Истина);
	Если НЕ ЗначениеЗаполнено(СвойствоНеВыгружатьВЗерно) Тогда
		ЗафиксироватьОшибку("Не найдено свойство ПВХ ""Не выгружать в ФГИС Зерно""!");
		Возврат;
	КонецЕсли;
	
	РазрезПоТоварам = Истина;
	
	Если РазрезПоТоварам Тогда
		...
		|ГДЕ
		|	НЕ РасходнаяНал.ПометкаУдаления
		|	И РасходнаяНал.Проведен
		|	И РасходнаяНал.Дата МЕЖДУ &НачПериода И &КонПериода
		|	И &ОтборПоФирме
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЖурналСтатусов.Дата КАК Дата,
		|	ВТ_РН.Документ КАК Документ,
		|	ЖурналСтатусов.Статус КАК Статус
		|ПОМЕСТИТЬ ВТ_Статусы
		|ИЗ
		|	ВТ_РН КАК ВТ_РН
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЖурналСтатусов КАК ЖурналСтатусов
		|		ПО (ЖурналСтатусов.Документ = ВТ_РН.Документ)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_Статусы.Документ КАК Документ,
		|	МАКСИМУМ(ВТ_Статусы.Дата) КАК Дата
		|ПОМЕСТИТЬ ВТ_СтатусыМаксДата
		|ИЗ
		...
		|ПОМЕСТИТЬ ВТ_РНОтобранные
		|ИЗ
		|	ВТ_Статусы КАК ВТ_Статусы
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СтатусыМаксДата КАК ВТ_СтатусыМаксДата
		|		ПО ВТ_Статусы.Дата = ВТ_СтатусыМаксДата.Дата
		|			И ВТ_Статусы.Документ = ВТ_СтатусыМаксДата.Документ
		|			И (ВТ_Статусы.Статус В (&СписокДопустимыхСтатусовДокументов))
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПериодическиеРеквизитыКлиенты.СрезПоследних КАК ПериодическиеРеквизитыКлиентыСрезПоследних
		|		ПО ВТ_Статусы.Документ.ЮрЛицо.Владелец = ПериодическиеРеквизитыКлиентыСрезПоследних.Объект
		|			И (ПериодическиеРеквизитыКлиентыСрезПоследних.Тип = ЗНАЧЕНИЕ(Справочник.ПериодическиеРеквизиты.Сотрудник))
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДополнительныеСведения КАК ДополнительныеСведения
		|		ПО ВТ_Статусы.Документ = ДополнительныеСведения.Объект
		|			И (ДополнительныеСведения.Свойство = &СвойствоНеВыгружатьВЗерно)
		|ГДЕ
		|	(ПериодическиеРеквизитыКлиентыСрезПоследних.ЗначениеРек ЕСТЬ NULL
		|			ИЛИ ПериодическиеРеквизитыКлиентыСрезПоследних.ЗначениеРек = ЛОЖЬ
		|			ИЛИ ПериодическиеРеквизитыКлиентыСрезПоследних.ЗначениеРек = 0)
		|	И ЕСТЬNULL(ДополнительныеСведения.Значение, ЛОЖЬ) = ЛОЖЬ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_РНОтобранные.Документ КАК Документ,
		|	ВТ_РНОтобранные.Статус КАК Статус,
		|	РасходнаяНалТабличнаяЧасть.Товар КАК Товар,
		|	РасходнаяНалТабличнаяЧасть.Количество * РасходнаяНалТабличнаяЧасть.Коэфф КАК Количество,
		|	РасходнаяНалТабличнаяЧасть.Товар.БазоваяЕдиница.ВесНетто КАК ВесНетто
		|ПОМЕСТИТЬ ВТ_Товары
		|ИЗ
		|	ВТ_РНОтобранные КАК ВТ_РНОтобранные
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РасходнаяНал.ТабличнаяЧасть КАК РасходнаяНалТабличнаяЧасть
		|		ПО ВТ_РНОтобранные.Документ = РасходнаяНалТабличнаяЧасть.Ссылка
		|			И (РасходнаяНалТабличнаяЧасть.Количество > 0)
		...
		|	МАКСИМУМ(ВТ_Товары.Статус) КАК Статус,
		|	МИНИМУМ(ЖурналСтатусов.Дата) КАК ДатаПервогоСтатуса,
		|	ВТ_Товары.Товар КАК Товар,
		|	МАКСИМУМ(ВТ_Товары.Товар.Поставщик) КАК Поставщик,
		|	МАКСИМУМ(ЮрЛица.Ссылка) КАК Производитель,
		|	МАКСИМУМ(ВТ_Товары.Количество) КАК Количество,
		|	МАКСИМУМ(ВТ_Товары.ВесНетто) КАК ВесНетто,
		|	ЕСТЬNULL(УпаковочныеЛисты.ДатаПроизводства, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаПроизводства,
		|	СУММА(ЕСТЬNULL(УпаковочныеЛисты.Количество, 0)) КАК КоличествоПоДатеПроизводства
		|ПОМЕСТИТЬ ВТ_ТоварыДатыПроизводства
		|ИЗ
		|	ВТ_Товары КАК ВТ_Товары
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УпаковочныеЛисты КАК УпаковочныеЛисты
		|		ПО ВТ_Товары.Документ = УпаковочныеЛисты.Документ
		|			И ВТ_Товары.Товар = УпаковочныеЛисты.Товар
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЮрЛица КАК ЮрЛица
		|		ПО (ВТ_Товары.Товар.Поставщик.ИНН <> """")
		|			И ВТ_Товары.Товар.Поставщик.ИНН = ЮрЛица.ИНН
		|			И (ВТ_Товары.Товар.Поставщик.ДатаВыбытия = ДАТАВРЕМЯ(1, 1, 1)
		|				ИЛИ ВТ_Товары.Товар.Поставщик.ДатаВыбытия > ВТ_Товары.Документ.Дата)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЖурналСтатусов КАК ЖурналСтатусов
		|		ПО ВТ_Товары.Документ = ЖурналСтатусов.Документ
		|			И (ЖурналСтатусов.Статус = &ПервыйСтатус)
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_Товары.Документ,
		|	ВТ_Товары.Товар,
		|	ЕСТЬNULL(УпаковочныеЛисты.ДатаПроизводства, ДАТАВРЕМЯ(1, 1, 1))
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ТоварыДатыПроизводства.Документ КАК Документ,
		|	ВТ_ТоварыДатыПроизводства.Товар КАК Товар,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТ_ТоварыДатыПроизводства.ДатаПроизводства) > 1 КАК НесколькоДатПроизводства,
		|	СУММА(ВТ_ТоварыДатыПроизводства.КоличествоПоДатеПроизводства) КАК КоличествоПоДатеПроизводстваИтого
		|ПОМЕСТИТЬ ВТ_ТоварыДатыПроизводстваГруппировка
		|ИЗ
		|	ВТ_ТоварыДатыПроизводства КАК ВТ_ТоварыДатыПроизводства
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ТоварыДатыПроизводства.Документ,
		|	ВТ_ТоварыДатыПроизводства.Товар
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ТоварыДатыПроизводства.Документ КАК Документ,
		|	ВТ_ТоварыДатыПроизводства.Статус КАК Статус,
		|	ВТ_ТоварыДатыПроизводства.ДатаПервогоСтатуса КАК ДатаПервогоСтатуса,
		|	ВТ_ТоварыДатыПроизводства.Товар КАК Товар,
		|	ВТ_ТоварыДатыПроизводства.Поставщик КАК Поставщик,
		|	ВТ_ТоварыДатыПроизводства.Производитель КАК Производитель,
		|	ВТ_ТоварыДатыПроизводства.ДатаПроизводства КАК ДатаПроизводства,
		|	ВТ_ТоварыДатыПроизводства.Документ.Склад.Фирма КАК Фирма,
		|	ВТ_ТоварыДатыПроизводства.Документ.ЮрЛицо КАК ЮрЛицо,
		|	ВТ_ТоварыДатыПроизводства.Документ.ЮрЛицо.ОГРН КАК ОГРН,
		|	ВТ_ТоварыДатыПроизводства.Документ.ЮрЛицо.ОКОПФ КАК ОКОПФ,
		|	ВТ_ТоварыДатыПроизводства.Товар.ОКПД2 КАК ОКПД2,
		|	ВТ_ТоварыДатыПроизводства.Документ.Сумма_Итог КАК СуммаДокумента,
		|	Зерно_Задачи.Ссылка КАК ЗадачаСДИЗ,
		|	Зерно_Задачи.Статус КАК СтатусЗадачи,
		|	ВЫБОР
		|		КОГДА ВТ_ТоварыДатыПроизводстваГруппировка.НесколькоДатПроизводства
		|			ТОГДА ВТ_ТоварыДатыПроизводства.КоличествоПоДатеПроизводства * ВТ_ТоварыДатыПроизводства.ВесНетто
		|		ИНАЧЕ ВТ_ТоварыДатыПроизводства.Количество * ВТ_ТоварыДатыПроизводства.ВесНетто
		|	КОНЕЦ КАК Масса,
		|	ВТ_ТоварыДатыПроизводстваГруппировка.НесколькоДатПроизводства
		|			И ВТ_ТоварыДатыПроизводства.Количество <> ВТ_ТоварыДатыПроизводстваГруппировка.КоличествоПоДатеПроизводстваИтого
		|		ИЛИ ВТ_ТоварыДатыПроизводства.ДатаПроизводства = ДАТАВРЕМЯ(1, 1, 1) КАК ЕстьОшибка,
		|	&Выбран КАК Выбран
		|ИЗ
		|	ВТ_ТоварыДатыПроизводства КАК ВТ_ТоварыДатыПроизводства
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Зерно_Задачи КАК Зерно_Задачи
		|		ПО ВТ_ТоварыДатыПроизводства.Документ = Зерно_Задачи.УПД
		|			И (НЕ Зерно_Задачи.ПометкаУдаления)
		|			И (ВТ_ТоварыДатыПроизводства.Товар.ОКПД2 = Зерно_Задачи.ОКПД2
		|					И ВТ_ТоварыДатыПроизводства.Производитель = Зерно_Задачи.Производитель
		|					И (ВТ_ТоварыДатыПроизводства.Товар = Зерно_Задачи.Товар
		|						ИЛИ Зерно_Задачи.Товар = ЗНАЧЕНИЕ(Справочник.Товары.ПустаяСсылка))
		|					И (ВТ_ТоварыДатыПроизводства.ДатаПроизводства = Зерно_Задачи.ДатаПроизводства
		|						ИЛИ Зерно_Задачи.ДатаПроизводства = ДАТАВРЕМЯ(1, 1, 1))
		|				ИЛИ Зерно_Задачи.GUID <> """")
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ТоварыДатыПроизводстваГруппировка КАК ВТ_ТоварыДатыПроизводстваГруппировка
		|		ПО ВТ_ТоварыДатыПроизводства.Документ = ВТ_ТоварыДатыПроизводстваГруппировка.Документ
		|			И ВТ_ТоварыДатыПроизводства.Товар = ВТ_ТоварыДатыПроизводстваГруппировка.Товар
		|ГДЕ
		|	(Зерно_Задачи.Ссылка ЕСТЬ NULL
		|			ИЛИ &ВключатьОбработанныеДокументы)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Фирма,
		|	Документ
	...
	Иначе
		...
		|ВЫБРАТЬ
		|	ВТ_РНОтобранные.Документ КАК Документ,
		|	МАКСИМУМ(ВТ_РНОтобранные.Статус) КАК Статус,
		|	РасходнаяНалТабличнаяЧасть.Товар.ОКПД2 КАК ОКПД2,
		|	РасходнаяНалТабличнаяЧасть.Товар.Поставщик КАК Поставщик,
		|	СУММА(РасходнаяНалТабличнаяЧасть.Количество * РасходнаяНалТабличнаяЧасть.Коэфф * РасходнаяНалТабличнаяЧасть.Товар.БазоваяЕдиница.ВесНетто) КАК Масса
		|ПОМЕСТИТЬ ВТ_Товары
		|ИЗ
		|	ВТ_РНОтобранные КАК ВТ_РНОтобранные
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РасходнаяНал.ТабличнаяЧасть КАК РасходнаяНалТабличнаяЧасть
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_РНОтобранные.Документ,
		|	РасходнаяНалТабличнаяЧасть.Товар.ОКПД2,
		|	РасходнаяНалТабличнаяЧасть.Товар.Поставщик
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВТ_Товары.ОКПД2 КАК ОКПД2,
		|	ВТ_Товары.Поставщик КАК Поставщик,
		|	ВТ_Товары.Масса КАК Масса,
		|	ЮрЛица.Ссылка КАК Производитель
		|ПОМЕСТИТЬ ВТ_ТоварыПроизводитель
		|ИЗ
		|	ВТ_Товары КАК ВТ_Товары
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЮрЛица КАК ЮрЛица
		|		ПО (ВТ_Товары.Поставщик.ИНН <> """")
		|			И ВТ_Товары.Поставщик.ИНН = ЮрЛица.ИНН
		|			И (ВТ_Товары.Поставщик.ДатаВыбытия = ДАТАВРЕМЯ(1, 1, 1)
		|				ИЛИ ВТ_Товары.Поставщик.ДатаВыбытия > ВТ_Товары.Документ.Дата)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ТоварыПроизводитель.Документ КАК Документ,
		|	ВТ_ТоварыПроизводитель.Статус КАК Статус,
		|	ВТ_ТоварыПроизводитель.ОКПД2 КАК ОКПД2,
		|	ВТ_ТоварыПроизводитель.Поставщик КАК Поставщик,
		|	ВТ_ТоварыПроизводитель.Производитель КАК Производитель,
		|	ВТ_ТоварыПроизводитель.Масса КАК Масса,
		|	ВТ_ТоварыПроизводитель.Документ.Склад.Фирма КАК Фирма,
		|	ВТ_ТоварыПроизводитель.Документ.ЮрЛицо КАК ЮрЛицо,
		|	ВТ_ТоварыПроизводитель.Документ.ЮрЛицо.ОГРН КАК ОГРН,
		|	ВТ_ТоварыПроизводитель.Документ.ЮрЛицо.ОКОПФ КАК ОКОПФ,
		|	ВТ_ТоварыПроизводитель.Документ.Сумма_Итог КАК СуммаДокумента,
		|	Зерно_Задачи.Ссылка КАК ЗадачаСДИЗ,
		|	Зерно_Задачи.Статус КАК СтатусЗадачи,
		|	&Выбран КАК Выбран
		|ИЗ
		|	ВТ_ТоварыПроизводитель КАК ВТ_ТоварыПроизводитель
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Зерно_Задачи КАК Зерно_Задачи
		|		ПО ВТ_ТоварыПроизводитель.Документ = Зерно_Задачи.УПД
		|			И (НЕ Зерно_Задачи.ПометкаУдаления)
		|			И (ВТ_ТоварыПроизводитель.ОКПД2 = Зерно_Задачи.ОКПД2
		|					И ВТ_ТоварыПроизводитель.Производитель = Зерно_Задачи.Производитель
		|				ИЛИ Зерно_Задачи.GUID <> """")
		|ГДЕ
		|	(Зерно_Задачи.Ссылка ЕСТЬ NULL
		|			ИЛИ &ВключатьОбработанныеДокументы)
	...
	Если ЗначениеЗаполнено(Параметры.Фирмы) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборПоФирме", "РасходнаяНал.Склад.Фирма В(&Фирмы)");
		Запрос.УстановитьПараметр("Фирмы", Параметры.Фирмы);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборПоФирме", "ИСТИНА");
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("СписокДопустимыхСтатусовДокументов", СписокДопустимыхСтатусовДокументов());
	Запрос.УстановитьПараметр("НетОтбораПоФирме", НЕ ЗначениеЗаполнено(Параметры.Фирмы));
	Запрос.УстановитьПараметр("ВключатьОбработанныеДокументы", Параметры.ВключатьОбработанныеДокументы);
	Запрос.УстановитьПараметр("СвойствоНеВыгружатьВЗерно", СвойствоНеВыгружатьВЗерно);
	Запрос.УстановитьПараметр("ПервыйСтатус", ПервыйСтатус());
...
Функция СоздатьСДИЗ(ЗадачаСДИЗ)
	...
	РезультатПараметрыСДИЗ = ПолучитьПараметрыСозданияСДИЗ(ЗадачаСДИЗ);
	...
	Если НЕ ЗапускИзФормы Тогда
		ОтпечатокСертификата = ПолучитьСертификат(ЗадачаСДИЗ.Фирма); // Определяем сертификат для подписания
	КонецЕсли;
...
Функция ВыделитьПартиюДляСДИЗ(ЗадачаСДИЗ)
	...
	Запрос.Текст = 
		...
		|				ИЛИ ВТ_СтруктураПоискаПартии.ДатаПроизводства = ДАТАВРЕМЯ(1, 1, 1))
	...
	// Добавляем в регистр новую партию?
...
Функция СоздатьПартиюИзДругихПартий(СписокПартий) Экспорт
	...
	Для Каждого Партия Из СписокПартий Цикл
		Если Партия.Статус <> "SUBSCRIBED" Тогда
			Сообщить("У партии """+Партия.НомерПартии+""" должен быть статус ""Подписана""!");
...
Функция ПогаситьСДИЗ(СДИЗ) Экспорт
	
	Параметры = Новый Структура();
	Параметры.Вставить("SDIZNumber", СДИЗ.Number);
	Параметры.Вставить("amount", СДИЗ.AmountOriginal-1);
	Параметры.Вставить("fullExtinction", Ложь);
	
	//РезультатОбъект = ВыполнитьЗапрос("CreateExtinction", Параметры);
	//
	//Если РезультатОбъект = Неопределено Тогда
	//	Сообщить("Не удалось погасить СДИЗ!");
	//	Возврат Неопределено;
	//КонецЕсли;
...
Процедура ПолучитьСправочник(ИмяСправочника) Экспорт
	...
	//Параметры.Вставить("Dictionary", "OKPD2");
	//Параметры.Вставить("Dictionary", "LotsTarget");
	//Параметры.Вставить("Dictionary", "LotsPurpose");
	//Параметры.Вставить("Dictionary", "QualityIndicator");
...
Функция ВыполнитьЗапрос(ТипЗапроса, Параметры)
	...
	Если РезультатЗапроса.Ошибка Тогда
		...
		//#Если Клиент Тогда
		//Сообщить(РезультатЗапроса.ТекстОшибки);
		//#КонецЕсли
...
Функция ОтправитьЗапрос(ТекстЗапросаXML)
	...
	КонвертXML = ПолучитьПодписанныйКонвертXML("SendRequestRequest", MessagePrimaryContent, MessageID, ReferenceMessageID);
	...
	Для ТекПопытка = 1 По КоличествоПопыток Цикл
		...
		//Сообщить("SendRequestRequest... Попытка: "+ТекПопытка); // Тест таймингов
	...
	MessageID = Строка(Новый УникальныйИдентификатор);
	КонвертXML = ПолучитьПодписанныйКонвертXML("SendResponseRequest", "", MessageID, ReferenceMessageID);
	HTTPЗапрос.УстановитьТелоИзСтроки(КонвертXML, КодировкаТекста.UTF8);
	...
	Для ТекПопытка = 1 По КоличествоПопыток Цикл
		...
		//Сообщить("SendResponseRequest... Попытка: "+ТекПопытка); // Тест таймингов
	...
	Если ЗначениеЗаполнено(ОтветныйMessageID) Тогда
		...
		КонвертXML = ПолучитьПодписанныйКонвертXML("AckRequest", "", MessageID, ReferenceMessageID);
		HTTPЗапрос.УстановитьТелоИзСтроки(КонвертXML, КодировкаТекста.UTF8);
		...
		Для ТекПопытка = 1 По КоличествоПопыток Цикл
			...
			//Сообщить("AckRequest... Попытка: "+ТекПопытка); // Тест таймингов
			...
			// Ошибку AckRequest осознанно никак не обрабатываем
...
Функция ЗапросСоздатьПартиюППЗИзОстатков(Параметры)
	...
	ManufacturerLegalEntity = ПолучитьДанныеКонтрагента(Параметры.Производитель);
...
Функция ЗапросСоздатьПартиюППЗИзДругихПартий(Параметры)
	...
	ManufacturerLegalEntity = ПолучитьДанныеКонтрагента(ПартияОснование.Производитель);
...
Функция ПолучитьПодписанныйКонвертXML(ТипЗапроса, MessagePrimaryContent, MessageID, ReferenceMessageID)
...
Функция ПолучитьДекларациюОСоответствии(ЗадачаСДИЗ)
...
Функция СоответствиеАдресовХраненияПартий()
	
	СоответствиеАдресовХраненияПартий = Новый Соответствие;
	
	Если ПараметрыСеанса.ПараметрыСеанса.ВидБазыТорговли = Справочники.ВидыБазТорговли.КОКАКОЛА Тогда
		Префикс = "КОКАКОЛА ";
	ИначеЕсли ПараметрыСеанса.ПараметрыСеанса.ВидБазыТорговли = Справочники.ВидыБазТорговли.ПЕПСИ Тогда
		Префикс = "ПЕПСИ ";
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	
	СоответствиеАдресовХраненияПартий.Вставить("Российская Федерация, Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1", Справочники.Фирмы.НайтиПоНаименованию(Префикс+"Улан-Удэ"));
	СоответствиеАдресовХраненияПартий.Вставить("Российская Федерация, Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1", Справочники.Фирмы.НайтиПоНаименованию(Префикс+"Благовещенск"));
	СоответствиеАдресовХраненияПартий.Вставить("Российская Федерация, Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1", Справочники.Фирмы.НайтиПоНаименованию(Префикс+"Камчатка"));
	СоответствиеАдресовХраненияПартий.Вставить("Российская Федерация, Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1", Справочники.Фирмы.НайтиПоНаименованию(Префикс+"Артем"));
	СоответствиеАдресовХраненияПартий.Вставить("Российская Федерация, Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1", Справочники.Фирмы.НайтиПоНаименованию(Префикс+"Якутск"));
	СоответствиеАдресовХраненияПартий.Вставить("Российская Федерация, Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1", Справочники.Фирмы.НайтиПоНаименованию(Префикс+"Нерюнгри"));
	СоответствиеАдресовХраненияПартий.Вставить("Российская Федерация, Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1", Справочники.Фирмы.НайтиПоНаименованию(Префикс+"Сахалин"));
	СоответствиеАдресовХраненияПартий.Вставить("Российская Федерация, Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1", Справочники.Фирмы.НайтиПоНаименованию(Префикс+"Чита"));
	СоответствиеАдресовХраненияПартий.Вставить("Российская Федерация, Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1", Справочники.Фирмы.НайтиПоНаименованию(Префикс+"Братск"));
	СоответствиеАдресовХраненияПартий.Вставить("Российская Федерация, Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1", Справочники.Фирмы.НайтиПоНаименованию(Префикс+"Хабаровск"));
	СоответствиеАдресовХраненияПартий.Вставить("Российская Федерация, Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1", Справочники.Фирмы.НайтиПоНаименованию(Префикс+"Иркутск"));
	СоответствиеАдресовХраненияПартий.Вставить("Российская Федерация, Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1", Справочники.Фирмы.НайтиПоНаименованию(Префикс+"Иркутск"));
	СоответствиеАдресовХраненияПартий.Вставить("Российская Федерация, Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1", Справочники.Фирмы.НайтиПоНаименованию(Префикс+"Совгавань"));
	
	Если ТестовыйСервер Тогда
		СоответствиеАдресовХраненияПартий.Вставить("Российская Федерация, Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1", Справочники.Фирмы.НайтиПоНаименованию(Префикс+"Хабаровск"));
		СоответствиеАдресовХраненияПартий.Вставить("123", Справочники.Фирмы.НайтиПоНаименованию(Префикс+"Хабаровск"));
	КонецЕсли;
	
	Возврат СоответствиеАдресовХраненияПартий;
	
КонецФункции

Функция СоответствиеАдресовПроизводителей()
	
	СоответствиеАдресовПроизводителей = Новый Соответствие;
	
	Если ПараметрыСеанса.ПараметрыСеанса.ВидБазыТорговли <> Справочники.ВидыБазТорговли.ПЕПСИ Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	СоответствиеАдресовПроизводителей.Вставить("Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1",
		Справочники.ЮрЛица.НайтиПоРеквизиту("ИНН", "9999999999")); // КОКАКОЛА
	СоответствиеАдресовПроизводителей.Вставить("Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1",
		Справочники.ЮрЛица.НайтиПоРеквизиту("ИНН", "9999999999")); // КОКАКОЛА
	СоответствиеАдресовПроизводителей.Вставить("Ханты-Мансийский автономный округ — Югра, г Ханты-Мансийск, ул Анны Коньковой, д 1",
		Справочники.ЮрЛица.НайтиПоРеквизиту("ИНН", "9999999999")); // КОКАКОЛА
	...
	Возврат СоответствиеАдресовПроизводителей;
...
Процедура УстановитьВремяЗапроса()
	...
	//сообщить(НоваяСтрока.ВремяЗапроса);
...
Функция ПолучитьДанныеКонтрагента(ЮрЛицо)
...
Функция ПреобразоватьДатуИзXML(XMLDate)
...
Функция ПолучитьManufacturer_Address(Производитель)
...
Функция ПолучитьСертификат(Знач Фирма = Неопределено)
...
Процедура ТестЛимитаЗапросов() Экспорт
	...
	MessageID = Строка(Новый УникальныйИдентификатор);
	КонвертXML = ПолучитьПодписанныйКонвертXML("AckRequest", "", MessageID, MessageID);
	HTTPЗапрос.УстановитьТелоИзСтроки(КонвертXML, КодировкаТекста.UTF8);
	...
	Для ТекПопытка = 1 По КоличествоПопыток Цикл
		...
		#Если Клиент Тогда
		Сообщить("Отправляем "+ТекПопытка+" запрос...");
		#КонецЕсли
		...
		Если СтрНайти(ОтветСтрока, "Отправитель заблокирован, превышен лимит сообщений в минуту") <> 0 Тогда
			...
			ВремяТеста = ТекущаяДата() - ВремяНачала;
			
			#Если Клиент Тогда
				Сообщить("Отправитель заблокирован! Итоговое количество запросов: "+ТекПопытка+" за "+ВремяТеста+" сек");
			#КонецЕсли
...
Процедура ПодписатьXML(КонвертXML)
	...
	//Отпечаток = "1ec8a2734610199c7add4f8459ae4125d2ddbfd4";
	
	Сертификат = ПолучитьСертификатПоОтпечатку(ОтпечатокСертификата, МенеджерКриптографии);
...
Функция ПолучитьСертификатПоОтпечатку(ОтпечатокСертификата = Неопределено, МенеджерКриптографии)
...
Функция ПолучитьПараметрыСозданияСДИЗ(ЗадачаСДИЗ)
	...
	ДопИнфо = ПолучитьДополнительнуюИнформациюДокумента(ДокументРН);
	...
	DeclarationOfConformit = ПолучитьДекларациюОСоответствии(ЗадачаСДИЗ);
	...
	Если Seller.Свойство("ТекстОшибки") Тогда
		...
		Возврат Результат;
	КонецЕсли;
	Seller.KPP = "999999999"; // Ставим КПП продавца такой же, как в системе
...
Функция ПолучитьДополнительнуюИнформациюДокумента(Документ)
	...
	Результат = Новый Структура();
	//Результат.Вставить("ContractDate", Формат(Выборка.ДоговорДата, "ДФ=dd.MM.yyyy"));
...
Функция РазобратьФИО(ЮрЛицо, ИмяИП = "")
	...
	Если ИмяИП <> "" Тогда
		...
		НаименованиеКонтрагента = Строка(ЮрЛицо);
		
	ИначеЕсли ТипЗнч(ЮрЛицо) = ТИп("СправочникСсылка.ЮрЛица") Тогда
	...
	Если не ЗначениеЗаполнено(НаименованиеКонтрагента) Тогда
		// либо не заполнено полное наименование либо передано чтото не имеющее представление строкового
		Возврат СтруктураОтвета;
	КонецЕсли;	
	
	//вырезаем ИП	
	НаименованиеКонтрагента = СтрЗаменить(НаименованиеКонтрагента,"""","");
	НаименованиеКонтрагента = СокрЛП(СтрЗаменить(НаименованиеКонтрагента,"Индивидуальный предприниматель",""));
	НаименованиеКонтрагента = СокрЛП(СтрЗаменить(НаименованиеКонтрагента,"ИП ",""));
	ПозИП = СтрНайти(НаименованиеКонтрагента,"ИП");
	
	Если ПозИП > 0 и ПозИП = СтрДлина(НаименованиеКонтрагента)-1 Тогда
		НаименованиеКонтрагента = СокрЛП(Лев(НаименованиеКонтрагента,ПозИП-1));
	КонецЕсли;
	
	
	МассивСлов = СтрРазделить(НаименованиеКонтрагента," ",Ложь);
	...
	Если МассивСлов.Количество()=3 Тогда
		...
		СтруктураОтвета.Вставить("Фамилия",Фамилия);
	...
	ИначеЕсли МассивСлов.Количество()>3 Тогда	
		...
		Отчество = СтрСоединить(МассивСлов," ");
	...
	ИначеЕсли МассивСлов.Количество()=2 И ИПБезОтчества(ЮрЛицо) Тогда
...
Функция ПроверитьЗаполнениеПолей(СтруктураКонтрагент, ЭтоИП, ЭтоConsignee)
	
	МассивПолей = Новый Массив;
	
	Если НЕ ЗначениеЗаполнено(СтруктураКонтрагент.OKOPF) Тогда
		МассивПолей.Добавить("ОКОПФ");
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(СтруктураКонтрагент.INN) Тогда
		МассивПолей.Добавить("ИНН");
	КонецЕсли;	
	Если НЕ ЗначениеЗаполнено(СтруктураКонтрагент.OGRN) Тогда
		МассивПолей.Добавить("ОГРН");
	КонецЕсли;		
	Если НЕ ЗначениеЗаполнено(СтруктураКонтрагент.Address) Тогда
		МассивПолей.Добавить("адрес");
	КонецЕсли;
	
	Если ЭтоИП Тогда
		Если НЕ ЗначениеЗаполнено(СтруктураКонтрагент.FirstName) Тогда
			МассивПолей.Добавить("имя");
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(СтруктураКонтрагент.FamilyName) Тогда
			МассивПолей.Добавить("фамилия");
		КонецЕсли;
	Иначе
		Если НЕ ЗначениеЗаполнено(СтруктураКонтрагент.Name) Тогда
			МассивПолей.Добавить("наименование организации");
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(СтруктураКонтрагент.KPP) Тогда
			МассивПолей.Добавить("КПП");
		КонецЕсли;
	КонецЕсли;
	...
	НезаполненныеПоля = СтрСоединить(МассивПолей, ", ");
	
	ТекстОшибки = "У "+?(ЭтоConsignee, "грузополучателя", "покупателя")+
	?(ЗначениеЗаполнено(СтруктураКонтрагент.Name), " """+СтруктураКонтрагент.Name+"""", "")+
	?(ЗначениеЗаполнено(СтруктураКонтрагент.INN), " (ИНН "+СтруктураКонтрагент.INN+")", "")+
	" не заполнены обязательные поля: "+НезаполненныеПоля;
	
	Возврат ТекстОшибки;
	
КонецФункции

Функция ПолучитьXmlTorg12Content(ДокССылка)
	
	Content = ИнициализироватьКонтентДляXmlTorg12();
	
	Параметр = Новый Структура;
	Параметр.Вставить("Контекст", ДокССылка);
	Параметр.Вставить("ВыводитьСразу",Истина);
	Параметр.Вставить("Количество",1);
	Параметр.Вставить("ПечатьСразу",Ложь);
	Параметр.Вставить("Принтер","");
	Параметр.Вставить("ДляПочты",Истина);
	Параметр.Вставить("КорректировкаИсправление", Неопределено);
	
	Обработки.ПечатьТОРГ12.ПечатьСчетаНакладной(Параметр, ДокССылка, Content, Неопределено);
...
Функция ИнициализироватьКонтентДляXmlTorg12() 
	...
	content.items 		= ПолучитьПустуюТЗдляItemsXmlTorg12();
	
	возврат Content;
	
КонецФункции

функция ИнициализироватьКонтентАдреса()  экспорт 
...
Функция ИнициализироватьКонтентКонтрагента(ИспользоватьТип=ложь) экспорт
	
	Если ИспользоватьТип Тогда
		ПоляТипа = "type";
	Иначе
		ПоляТипа = "IsSoleProprietor";
	КонецЕсли;
	СписокПолей = ПоляТипа+", BoxId, Name, Inn, Kpp,  Address, FnsParticipantId, BankAccountNumber, BankName, BankId, Phone, Okopf, Okpo";
...
Функция ПолучитьПустуюТЗдляItemsXmlTorg12()
	
	результат = новый ТаблицаЗначений;
	СтруктураКолонок = новый структура("Product, UnitCode, Quantity, Price, TaxRate, TotalWithVatExcluded, Vat, Total, Feature, Sort, Article, ProductCode, UnitName, ParcelType, ParcelCapacity, ParcelsQuantity, GrossQuantity, AdditionalInfo, СсылкаНаЕИ"); 
