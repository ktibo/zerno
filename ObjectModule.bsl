...
Перем АдресСервера;
Перем АдресТестовогоСервера;
Перем РесурсНаСервере;
...
Функция ОтправитьЗапрос(ТекстЗапросаXML)
	...
	Если ТестовыйСервер Тогда
		Адрес = АдресТестовогоСервера;
	КонецЕсли;
	
	Если HTTP = Неопределено ИЛИ HTTP.Сервер <> Адрес Тогда
		HTTP = Новый HTTPСоединение(Адрес, , , , , 60, Новый ЗащищенноеСоединениеOpenSSL());
	КонецЕсли;
...
АдресТестовогоСервера = "demo-zerno.fors.ru";
